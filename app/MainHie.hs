...
run opts = do
  ...
  withLogFun $ do
    ...
    let dispatcherProc =                void $ forkIO $ runIdeM ghcModOptions (IdeState plugins Map.empty Nothing) (dispatcher cin)
    let dispatcherProcP dispatcherEnv = void $ forkIO $ runIdeM ghcModOptions (IdeState plugins Map.empty Nothing) (dispatcherP dispatcherEnv pin)
