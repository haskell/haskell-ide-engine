...
hareSpec = do
  describe "hare plugin commands" $ do
    ...
    it "renames" $ do
      ...
      r `shouldBe` Just (IdeResponseOk (H.fromList ["refactor" .= ["test/testdata/HaReRename.hs"::FilePath]]))
    ...
    it "converts if to case" $ do
      ...
      r <- dispatchRequest req
      r `shouldBe` Just (IdeResponseOk (H.fromList ["refactor" .= ["test/testdata/HaReCase.hs"::FilePath]]))
    ...
    it "lifts one level" $ do
      ...
      r <- dispatchRequest req
      r `shouldBe` Just (IdeResponseOk (H.fromList ["refactor" .= ["test/testdata/HaReMoveDef.hs"::FilePath]]))
