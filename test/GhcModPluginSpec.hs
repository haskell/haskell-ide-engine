...
ghcmodSpec = do
  describe "ghc-mod plugin commands" $ do
    it "runs the check command" $ do
      ...
      r `shouldBe` IdeResponseOk (H.fromList ["ok" .= ("test/testdata/FileWithWarning.hs:4:7:Not in scope: \8216x\8217\n"::String)])
    ...
    it "runs the lint command" $ do
      ...
      r <- dispatchRequest req
      r `shouldBe` IdeResponseOk (H.fromList ["ok" .= ("./test/testdata/FileWithWarning.hs:6:9: Error: Redundant do\NULFound:\NUL  do return (3 + x)\NULWhy not:\NUL  return (3 + x)\n"::String)])
    ...
    it "runs the find command" $ do
      ...
      r <- dispatchRequest req
      r `shouldBe` IdeResponseOk (H.fromList ["ok" .= ("Placholder:Need to debug this in ghc-mod, returns 'does not exist (No such file or directory)'"::String)])
    ...
    it "runs the info command" $ do
      ...
      r <- dispatchRequest req
      r `shouldBe` IdeResponseOk (H.fromList ["ok" .= ("main :: IO () \t-- Defined at test/testdata/HaReRename.hs:2:1\n"::String)])
    ...
    it "runs the type command, correct params" $ do
      ...
      r <- dispatchRequest req
      r `shouldBe` IdeResponseOk (H.fromList ["ok" .= ("5 9 5 10 \"Int\"\n5 9 5 14 \"Int\"\n5 1 5 14 \"Int -> Int\"\n"::String)])
