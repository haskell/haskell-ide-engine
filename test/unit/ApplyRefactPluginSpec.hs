  ...
  describe "apply-refact plugin commands" $ do
    ...
    it "applies one hint only" $ do
      ...
      let furi = applyRefactPath
          ...
          res = return $ WorkspaceEdit (Just $ H.singleton applyRefactPath textEdits) Nothing
    ...
    it "returns hints as diagnostics" $ do
      ...
      let act = lintCmd' arg
          arg = applyRefactPath
          res = return $ PublishDiagnosticsParams
    ...
    it "respects hlint pragmas in the source file" $ do
      ...
      r <- runIGM testPlugins $ runIDErring req
      r `shouldBe` Right
    ...
    it "respects hlint config files in project root dir" $ do
      ...
      let req = lintCmd' filePath
      r <- cdAndDo "./test/testdata" $ runIGM testPlugins $ runIDErring req
      r `shouldBe` Right
           (PublishDiagnosticsParams
